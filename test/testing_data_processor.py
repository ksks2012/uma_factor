...
def test_total_factor():
    ...
    parent_two_info = {}
    try:
        horse_info = sqlite_instance.search_horse(f"SELECT * FROM HorseData WHERE horse_id = {horse_id}")
        horse_info = sql_data_to_horse_info(horse_info)[0]
        print(horse_info)
        parent_one_info = sqlite_instance.search_horse(f"SELECT * FROM HorseData WHERE horse_id = {int(horse_info['parent_one_id'])}")
        parent_one_info = sql_data_to_horse_info(parent_one_info)[0]
        print(parent_one_info)
        parent_two_info = sqlite_instance.search_horse(f"SELECT * FROM HorseData WHERE horse_id = {int(horse_info['parent_two_id'])}")
